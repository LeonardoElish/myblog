  ...
  help = () => {
    const help = (
      <ul className="list-disc ml-6 pb-1.5">
        <li>
          <span text-red-400>cat {"<文件>"}</span> - 查看文件内容
        </li>
        <li>
          <span text-red-400>cd {"<目录>"}</span> - 进入目录，"cd .." 返回上级，"cd" 或 "cd ~" 返回根目录
        </li>
        <li>
          <span text-red-400>ls</span> - 查看当前目录下的文件和文件夹
        </li>
        <li>
          <span text-red-400>clear</span> - 清屏
        </li>
        <li>
          <span text-red-400>help</span> - 显示帮助菜单
        </li>
        <li>
          <span text-red-400>rm -rf /</span> - :)
        </li>
        <li>
          按 <span text-red-400>上/下箭头</span> - 浏览历史命令
        </li>
        <li>
          按 <span text-red-400>tab</span> - 自动补全
        </li>
      </ul>
    );
      ...
      <div key={`terminal-input-row-${id}`} flex>
        <div className="w-max hstack space-x-1.5">
          <span text-yellow-200>
            Leonardo@macbook-pro <span text-green-300>{this.getCurDirName()}</span>
          </span>
  ...
  render() {
    return (
      ...
      >
        ...
        <div p="y-2 x-1.5">
          <span className="text-green-300">ヽ(ˋ▽ˊ)ノ</span>：嘿，你发现了终端！输入 `help` 开始探索。
        </div>
